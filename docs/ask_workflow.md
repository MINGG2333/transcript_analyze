...
- **Prompt 逻辑**：
  1. **硬编码排除** "陈嘉仪"——必须用"她"代替或直接移除（数据库每条都有该主播名，无区分力）；其他人名则必须保留（频率低、区分度高）
  2. **语义扩展**——根据问题类型用常识联想相关概念/场景融入改写句中
  3. **输出完整句子**——多个句子用分号连接
  4. **禁止编造**——不确定的具体信息不要写入，宁可泛化
  5. **输出精简**——控制在50字以内
...
- **Prompt 逻辑**：
  1. **硬编码排除** "陈嘉仪"——必须从关键词中删除（数据库每条都有，无区分力）；其他人名则必须保留
  2. **优先用名词/名物词**——通用动词（"是""有""做""说"等）不放入
  3. **大胆联想，沿四条路径扩展**：
     - ① **同义扩展**：同义词、近义词、口语/方言说法
     - ② **下位扩展**：抽象类别的具体实例/成员
     - ③ **场景扩展**：常一起出现的人、事、物、地点
     - ④ **取值扩展**：询问属性值时列出所有可能取值
  4. **基于可靠常识联想**，不编造无法确认的具体事实
  5. **最后一步：剔除宽泛词**——含义宽泛、任何话题下都高频出现的词（疑问词、代词、多义词等）要删除
  6. **最终输出多词组合**，词之间用空格分隔，不限制数量
...
- **并发日志清晰化**：每个视频分组使用 `PrefixedLogger` 包装原始 logger，自动在每条日志前加 `[视频索引/总数 视频标题]` 前缀，一眼区分并发输出归属
- **输入**：
  - 用户问题
...
- **输出（局部结果）**：
  ...
  ```
- **批内重试**：引用一致性校验不通过时，最多重试 5 次修正（每次重试调用 LLM），重试时同样启用思考模式
- **evidence 数量约束**：每个批次的 LLM 调用要求 evidence 条目数不超过 10 条
...
| # | 调用环节 | 函数位置 | description | max_tokens | 思考模式 | Prompt 字数限制 | 其他配置 |
|---|----------|----------|-------------|-----------|-------------------|----------------|----------|
| **1** | 检索(a′) — 知识库描述生成 | `qa_retrieval.py:95` `generate_kb_description()` | "知识库描述生成" | **300** | **启用**（默认） | 输出描述 20~50 字 | 仅在构建/更新知识库时触发，非每次问答 |
| **2** | 检索(b) — 向量查询改写 | `qa_retrieval.py:475` `refine_vector_query()` | "向量查询改写" | **300** | **启用**（默认） | 改写查询 ≤50 字 | 默认重试 5 次；失败回退到原始问题 |
| **3** | 检索(d) — BM25 查询改写 | `qa_retrieval.py:509` `refine_bm25_query()` | "BM25 查询改写" | **300** | **启用**（默认） | 改写查询 ≤50 字 | 默认重试 5 次；失败回退到原始问题 |
| **4** | 合成(B) — 分组合成（每批次首次） | `qa.py:186` `_process_video_group()` | `"直播 {live_id} 合成 {batch_n}/{total}"` | **None**（不限制） | **启用**（默认） | evidence ≤10 条；无严格字数限制 | 按 500 条/批切分；含后续引用一致性校验 |
| **5** | 合成(B) — 分组合成（引用修正重试） | `qa.py:254` `_process_video_group()` | `"直播 {live_id} 合成 {batch_n}/{total}（引用修正 第N次）"` | **None**（不限制） | **启用**（默认） | 同上 | 最多重试 5 次修正；修正后仍不通过则跳过该批次 |
| **6** | 合并(D) — 最终答案合并（首次） | `qa.py:611` `ask()` | "最终答案合并" | **None**（不限制） | **启用**（默认） | 输出答案 ≤300 字 | 仅当 ≥2 个视频分组有答案时触发 |
| **7** | 合并(D) — 最终答案合并（引用格式修正重试） | `qa.py:648` `ask()` | `"最终答案合并（引用格式修正 第N次）"` | **None**（不限制） | **启用**（默认） | 同上 | 最多重试 5 次修正 |
| **8** | 安全审核 — 内容安全分级判定 | `qa_safety.py:109` `check_content_safety()` | "内容安全审核" | **300** | **启用**（默认） | reason ≤50 字 | 返回四级风险等级（0~3） |

> **注意**：思考模式**全部默认启用**。当前所有 8 处调用均未显式禁用，因此全部启用思考模式。
> 如果后续发现某处调用不需要思考模式，可在对应 `call_llm_json()` 调用中传入关闭参数。
...
| #1 (向量查询改写) | 2 | 检索阶段 b |
| #2 (BM25查询改写) | 3 | 检索阶段 d |
| #3, #4... (分组合成) | 4 | 每批次一次，可多批次 |
| #N+1 (最终答案合并) | 6 | 仅多视频时有 |

### call_llm_json 核心参数（默认值）

所有调用均使用 `response_format={"type": "json_object"}` 强制 JSON 输出，`temperature=0`。

### JSON 解析失败重试
- JSON 解析失败（`json.JSONDecodeError`）时**只重试 1 次**，不执行指数退避
- 因为 JSON 解析失败通常意味着输出截断，重试相同 prompt 大概率同样结果，仅防瞬时故障
- 第 1 次失败后尝试用字符串提取 `{...}` 内容再次解析，如仍失败则重试 1 次
- 重试后仍失败则直接报错 `RuntimeError`

### API 错误重试策略
| 错误类型 | 是否重试 | 退避策略 |
|---|---|---|
| 429 RateLimit | ✅ | `2^attempt + random(0,1)`，优先用 Retry-After 头 |
| 500/503 服务器错误 | ✅ | `2^attempt` |
| APITimeout/APIConnectionError | ✅ | `2^attempt` |
| 400/401/402/422 | ❌ 不重试 | 直接报错 |
...
## BM25 查询改写 Prompt 演变对照

> 对比原始版本 (commit d136833) 与当前版本的 `build_bm25_refinement_prompt`

| 要素 | 原始版本 | 当前版本 | 状态 |
|------|---------|---------|:----:|
| 排除主播名 | 笼统"主播名/嘉宾名" | **硬编码"陈嘉仪"** | ✅ 更强 |
| 保留其他人名 | 无 | **明确要求保留** | ✅ **新增** |
| 动词过滤 | ✅ | ✅ | 保留 |
| 同义/近义联想 | ① | ① | 保留 |
| 下位具体实例 | ②（有例子） | ②（有例子） | 保留 |
...
| 不要猜具体名字 | ⑤ | **已删除** | 用户要求去掉 |
| **地点联想** | ⑥ 方言/食物/文化 | ⑤ **已恢复** | ✅ |
| 常识例子 | 男/女, 四川/重庆... | 性别→男/女, 祖籍→各地名 | ✅ 类似 |
| 宽泛词剔除 | 无 | **有，含具体列表** | ✅ **新增** |
| 字数限制 | "不限制数量" | **50字以内** | 用户要求 |
| 思考限制 | 无 | **≤300字** | ✅ **新增** |
| 违规警告 | 无 | ⚠️ **违规导致检索完全失败** | ✅ **新增** |
...
| **并发日志分组清晰度** | ❌ 不适用 | ✅ PrefixedLogger加 `[索引/总数 标题]` 前缀 | 并发输出时一眼区分归属 |
...
| JSON 解析失败重试 | ✅ | 仅重试 1 次（无指数退避），防瞬时故障 |
| 思考模式区分 | ✅ | 合成阶段启用，改写/审核/描述生成禁用 |
| PrefixedLogger | ✅ | 并发日志前缀清晰化 |
| evidence 数量约束 | ✅ | 每批 evidence ≤ 10 条 |
