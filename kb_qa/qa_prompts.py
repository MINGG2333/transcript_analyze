...
ANSWER_STYLE_RULES = (
    ...
    "💡 人称使用：在回答中请用「陈嘉仪」「嘉仪」「cjy」或粉丝昵称来称呼她，不要称其为「主播」。\n"
    "用喜欢她的人的视角来介绍，像在跟朋友安利一样自然地提及她的名字。\n\n"
)
...
SOURCE_DISTINCTION_RULES = (
    "注意区分不同来源的角色：\n"
    "- 「主播讲话」类型：标记为主播说话的片段，但转录时未区分声纹，可能包含主播周围的人或连麦对象的声音，不一定是主播本人说的。主播也可能在说话中**引用/转述他人话语**，需结合上下文分辨。\n"
    ...
    "⚠️ 关于弹幕的特别提醒——注意区分弹幕是在说主播还是在说自己：\n"
    "观众弹幕有些是在评论主播，有些则是在说观众自己的经历/感受/意见。\n"
    "即使主播随后说了\"是啊\"\"对\"\"嗯\"等简短回应，也不一定是在确认那条弹幕的内容——\n"
    "主播可能在同时回应多条弹幕，或只是随口附和。\n"
    "**优先采信主播主动、完整陈述的内容作为证据**，对弹幕及主播的简短回应请结合上下文谨慎判断。\n\n"
    "⚠️ 直播礼物语理解——主播说\"谢[粉丝名]送的[礼物名]\"是主播在念/感谢粉丝在直播间赠送的**虚拟礼物**，"
    "不是真实物品的赠送或转交。\"谢\"=感谢，\"送\"=在平台赠送虚拟道具。切勿将其理解为\"某人送了真实物品给主播\"。\n"
    "⚠️ \"后挂\"——直播互动用语，指连麦时后挂断的一方，延伸指后离开直播间的人。\n\n"
)
...
REASONING_RULES = (
    ...
    "以及本 prompt 中「补充参考信息」部分提供的公开资料，结合片段上下文来判断主播话语的真实含义。\n"
    ...
    return [
        {
            ...
            "content": (
                ...
                "第一步：如果用户问题中包含非中文内容，先将其翻译为中文（特殊标识不翻译）。\n"
                "第二步：对中文问题进行分析和关键词提取。\n\n"
                f"数据库描述：{kb_description}\n\n"
                "⚠️ **严格遵守以下规则，违规会导致检索完全失败：**\n\n"
                "1) **主播名\"陈嘉仪\"禁止出现**——它在数据库中每条都有，对缩小范围毫无帮助。\n"
                "   用户问题中问到的其他人名则**必须保留**。\n"
                "2) **通用动词禁止出现**——只有低频专用动词才可保留（如\"考研\"\"退团\"）；"
                "通用动词（是、有、做、说、去、来等）一律删除。\n"
                "3) **先判断是否需要联想**——如果问题中已包含高区分度的具体词（如人名、地名、具体事件名等），"
                "仅保留这些词即可，不需要联想通用关系词。只有问题中**缺乏具体关键词**时（如仅问\"祖籍\"\"老家\"\"年龄\"等），"
                "才需要大胆联想补充具体词，沿着以下思路：\n"
                "   ① 同义/近义/口语说法：概念有哪些其他说法？\n"
                "   ② 下位具体实例：抽象类别有哪些具体成员？（性别→男/女；祖籍→各地名）\n"
                "   ③ 场景联想：经常一起出现的人、事、物、地点？\n"
                "   ④ 取值列举：询问属性值时，列出所有可能的取值。\n"
                "   ⑤ 如果涉及地点，联想可能的方言、食物、风俗文化等地域相关词。\n"
                "4) 基于常识放心联想，不要自我审查。\n"
                "5) **宽泛词必须删除**——检查上面列出的每个词，如果它在各种话题下都会高频出现"
                "（疑问代词（单独用）、疑问形容词（加名词用）、代词、多义词、歧义词，以及朋友、关系、队友、闺蜜等），**一律删除**。\n"
                "6) 最终输出剩余的关键词（空格分隔），控制在50字以内。\n"
                "⚠️ 思考不超过200字。\n\n"
                f"用户问题：{question}\n"
    ...
    return [
        {
            ...
            "content": (
                ...
                f"数据库描述：{kb_description}\n\n"
                "⚠️ **严格遵守以下规则，违规会导致检索完全失败：**\n\n"
                "1) **主播名\"陈嘉仪\"禁止出现**——先正常组织问句，然后删掉其中的\"陈嘉仪\"。"
                "其他人名则**必须保留**。\n"
    ...
    return [
        {
            ...
            "content": (
                ...
                "  • 如果主播讲话内容与附近的弹幕内容相似或直接回应了弹幕，则可能是在**念/转述弹幕**。\n"
                "  • 请将你的判断写入每条有用片段的 reason 字段，注明「主播自述」还是「转述」或「疑似他人声音」。\n\n"
                "⚠️ 关于弹幕的特别提醒：弹幕有些在评论主播，有些在说观众自己。\n"
                "即使主播随后说\"是啊\"\"对\"等简短回应，也不一定是在确认那条弹幕——\n"
                "主播可能在同时回复多条弹幕，或随口附和。\n"
                "**优先采信主播主动、完整的陈述**，对弹幕及主播的简短回应请结合上下文谨慎判断。\n\n"
                f"{bg_text}\n\n"
                "🧠 通用推理原则：\n"
                "1) 请充分利用你训练数据中的**背景常识**（如公众人物的公开身份、团体的公开性质等已知信息），"
                "结合片段上下文来判断主播话语的真实含义。\n"
    ...
    return [
        {
            "role": "user",
            "content": (
                "你是一名熟知陈嘉仪的粉丝，现在请根据下面提供的直播片段来回答问题。\n\n"
                ...
                "2) evidence只包含**为答案提供了独特信息**的片段，不要为了凑数而列出不相关的片段；\n"
                ...
                "6) **输出精简**——evidence 条目数不要超过 10 条。如果多个 evidence 的论证角度高度相似，只保留其中最具代表性的 2-3 条即可，不要重复列出一堆角度相同的引用。如果片段中无直接相关的信息，evidence 应返回空列表 `[]`，不要在 answer 中引用任何编号。\n"
                "7) **仅输出JSON，不要额外文本**。"
            ),
        }
    ]
