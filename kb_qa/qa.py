...
from .models import RiskLevel, Segment
        ...
        safety_prompt = [
            {
                ...
                "content": (
                    ...
                    f"{citation_summary}\n\n"
                    "请输出JSON对象：\n"
                    '{"level": 0~3, "reason": "判断理由（中文，说明归入该等级的关键依据）"}\n'
    ...
    def ask(
        ...
        if len(answer_videos) == 0:
            ...
            for idx, vr in enumerate(answer_videos, start=1):
                ...
                # 列出该分组实际引用的所有 citations（使用全局编号），包含 reason 等分析信息
                evidence_lines = []
            ...
            merge_prompt = [
                {
                    ...
                    "content": (
                        ...
                        "7) ⚠️ 重要——请仔细阅读每条引用片段的「⚠️ 分析」字段，其中包含了该片段与问题关系的判断以及上下文说明（如：是否为主播自述/转述、是否确定与问题相关、是否有其他上下文限制等）。"
