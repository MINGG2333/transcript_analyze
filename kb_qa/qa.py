...
class VideoKnowledgeQA:
        ...
        return [
            {
                "role": "user",
                "content": (
                    "你是一个中文检索查询优化助手。请把下面的用户问题改写成一个或多个关键词，用于BM25检索。规则：只保留问题中关键部分的核心命名实体（不要包括采访者名字），去掉疑问词、助词和无关表达。若该名称由重复的单一汉字组成（如“顺顺”），则需同时输出该单字和完整名称。基本原则是用这些关键词搜索到的文本范围内会有问题的答案，关键词越少越好，每增加一个关键词，需要缩小搜索范围而不是扩大。\\n"
                    f"用户问题：{question}\n"
    ...
    def _build_group_query(self, questions: list[dict[str, str]]) -> str:
        ...
        return "请根据受访者（受访者不是GUO, An，GUO, An是采访者）的话回答以下相关子问题：\n" + "\n".join(lines)
